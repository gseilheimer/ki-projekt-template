...
//TODO: Pfad anpassen
//-----------------------------------
const PATH_ROOT = '/pwa00/C_ML'; // pathRoot bezeichnet den Pfad vom Server-Stammverzeichnis aus //'/ws/chopen2019/prj00/uXY_ML'
const URLS_TO_CACHE = [
    '',
...
// Ab hier: optionale Aufgabe D4: Push-Benachrichtigungen
//-----------------------------------

// Event Handler für eingehende Push-Nachrichten
self.addEventListener('push', ev => {
    console.log('ServiceWorker: Push-Nachricht empfangen');
    ...
    const options = {
        ...
        icon: data.icon || '/assets/media/icon-192.png',
        badge: '/assets/media/icon-48.png',
...
// Event Handler für Notification-Klicks
self.addEventListener('notificationclick', ev => {
    console.log('ServiceWorker: Notification geklickt', ev.notification.tag);

    ev.notification.close();

    // Optional: App öffnen wenn auf Benachrichtigung geklickt wird
    ev.waitUntil(
        clients.matchAll({ type: 'window', includeUncontrolled: true })
            .then(clientList => {
                // Wenn bereits ein Fenster offen ist, fokussiere es
                for (let client of clientList) {
                    if (client.url.includes(PATH_ROOT) && 'focus' in client) {
                        return client.focus();
                    }
                }
                // Ansonsten öffne neues Fenster
                if (clients.openWindow) {
                    return clients.openWindow(PATH_ROOT);
                }
            })
    );
});

// Event Handler für Notification-Schließen
self.addEventListener('notificationclose', ev => {
    console.log('ServiceWorker: Notification geschlossen', ev.notification.tag);
});